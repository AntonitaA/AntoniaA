...
class Database:
    ...
    # added column_name as argument to recognize the column
    # indexes
    def create_index(self, index_name, table_name, column_name, index_type='btree'):
        ...
        #    if self.tables[table_name].pk_idx is None:  # if no primary key, no index
        #        raise Exception('Cannot create index. Table has no primary key.')
        #elif self.tables[table_name].column_by_name(column_name)==self.tables[table_name].column_by_name(self.tables[table_name].pk):
        #    print("correct")
