...
class Table:
    ...
    def _insert(self, row, insert_stack=[]):
        ...
        #len(self.column_names) != len(self.column_types)
    ...
    def _select_where(self, return_columns, condition=None, distinct=False, order_by=None, desc=True, limit=None):
        ...
        if condition is not None:
            if "BETWEEN" in condition or "between" in condition:
                ...
                return_elements = []
                if split_cond[3] != "and":
                    print("The query BETWEEN needs the operator and to separate the arguments. Try again!")
                    exit()
                else:
                    value1 = split_cond[2] # first number or string in between condition
                    value2 = split_cond[4] # second number or string in between condition
                    column_cond = split_cond[0]
                    if type(value1) == str and type(value2) == str and not value1.isdigit() and not value2.isdigit():
                        column = self.column_by_name(column_cond)
                        #print("column from table: ")
                        #print(column)
                        sorted_column = sorted(column)
                        #print("column from table sorted: ")
                        #print(sorted_column)
                        index_start = sorted_column.index(value1)
                        #print("index start: ")
                        #print(index_start)
                        index_stop = sorted_column.index(value2)
                        #print("index stop: ")
                        #print(index_stop)
                        for i in range(index_start+1, index_stop):
                            #print(i)
                            #print(sorted_column[i])
                            for ind, x in enumerate(column):
                                if column[ind] == sorted_column[i]:
                                    return_elements.append(ind)
                       # print(return_elements)
                        rows = return_elements
                    elif value1.isdigit() and value2.isdigit():
                        column = self.column_by_name(column_cond)
                        print("column from table: ")
                        print(column)
                        for ind, x in enumerate(column):
                            if float(value1) <= x <= float(value2):
                                return_elements.append(ind)
                        print(return_elements)
                        rows = return_elements
